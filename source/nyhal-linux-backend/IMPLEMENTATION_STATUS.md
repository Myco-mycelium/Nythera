...
| IPC Semantics | `ipc/core.py`, `ipc/transport.py` | ✓ Implemented | send/receive/call/notify primitives, receive-side capability check, token-bucket rate limiting, **Unix-domain datagram transport** (receiver-side `SO_PASSCRED` — the kernel attaches the real `SCM_CREDENTIALS` to every inbound datagram and the sender attaches nothing, so identity is unforgeable and container-safe; wire-codec framing), **verified container→service end-to-end** |
