...
| REQ-SEC-0003 | Verifying a `.nygi` image SHALL include verifying a publisher signature covering the manifest and the content integrity trees (NPS-026 §6); checksum-only verification SHALL NOT be treated as establishing authenticity. | NPS-006 §6.3 / NPS-026 §6 | Verified | — | No installer/package-manager implementation exists yet (2026-08-12) — spec-level requirement from threat model Phase 7 (NPS-027, FIND-PACKAGE-002) |
| REQ-SEC-0004 | Publisher public keys SHALL be user-enrollable and revocable, and the first-install trust decision SHALL be an explicit, user-visible enrollment action — never silent trust-on-first-use of an unauthenticated key. | NPS-027 §3 (FIND-PACKAGE-003) | Verified | — | Design decided 2026-09-21 (AG decision log D3 — the ADR-0014 mirror); mechanism normative in NPS-026 v1.2.0 §6.3 — **closed** |
| REQ-SEC-0005 | Integrity verification of a mounted `.nygi` SHALL distinguish base-image content from overlay content and SHALL report the provenance of the combined view; unsigned overlay content SHALL shadow signed base content only for platform-designated user-modifiable classes. | NPS-006 §6.4 | Verified | — | Spec-level requirement from threat model Phase 7 (NPS-027, FIND-PACKAGE-004) |
| REQ-SEC-0006 | Every package verification, install, and uninstall event SHALL be recorded in the tamper-evident audit log (ADR-0018) with package identity, publisher, version, and verification result. | NPS-006 §6.5 | Verified | — | Spec-level requirement from threat model Phase 7 (NPS-027, FIND-PACKAGE-005) |

## Coverage Note

40 requirements across all 17 domain prefixes defined in NPC-009 §4 — at
