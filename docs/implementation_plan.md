...
*   **Network namespaces — LANDED 2026-08-14**: `ContainerConfig.network=True` (opt-in, default off) creates the container's own network namespace via `CLONE_NEWNET` (direct-syscall path) or `unshare --net` (legacy path), giving the container a loopback-only isolation boundary — no host interfaces are visible. **The launcher brings `lo` up before exec** (best-effort `SIOCSIFFLAGS`; succeeds in an owned netns, EPERMs harmlessly when sharing the host's), so a network container has a usable 127.0.0.1. Outbound connectivity (veth/bridge pairs) is deliberate future work — it requires host CAP_NET_ADMIN/root and is deferred; the seccomp data plane continues to gate `socket`/`connect`/`bind` on capability grants.
*   **PID-1 launcher-init — LANDED 2026-08-14**: the launcher no longer execs the container command; it stays alive as the namespace's PID-1 init, forks the command as a plain child, forwards supervisor signals, reaps it, and exits with its status (or dies by its signal). Rationale: Linux discards signals sent to a namespace PID 1 without a handler, so a command running AS PID 1 could never be terminated gracefully (the 10s SIGTERM window always elapsed). The init runs unfiltered (the model tini uses) and the seccomp policy is applied by the command child before its exec — so restricted containers cannot EPERM the init's own fork. The manager resolves the command's HOST pid via the init's `/proc` children file (ns-local pids are useless to the host side); both pids join the container's cgroups, and terminate escalation covers both.
...
*   **Custom IPC Mechanism**: Design a custom IPC mechanism, likely based on Unix domain sockets or a shared memory approach, to implement the `send`/`receive`/`call`/`notify` primitives. This will allow for efficient and secure communication between Nyrqis components. **Unix-domain datagram transport — LANDED 2026-08-14** (`ipc/transport.py`): `IPCMessage`s move between processes over `AF_UNIX SOCK_DGRAM` sockets framed by the wire codec (ADR-0020 migration #4), with **purely receiver-side kernel identity** — the sender attaches nothing, and `SO_PASSCRED` makes the kernel attach the real `SCM_CREDENTIALS` `(pid, uid, gid)` to every inbound datagram, so the backend maps the attached pid to a container and forged/unknown senders and senders lacking `CAP_IPC_SEND` are dropped before delivery (verified with a real cross-process exchange). `CALL`/`REPLY` works over sockets via `metadata['reply_path']` correlation. **Verified container→service end-to-end 2026-08-14:** a real `network=True` container with the network + filesystem caps runs an `IPCClient` under the active seccomp filter and completes a kernel-authenticated CALL/REPLY with a host-side service (`test_container_ipc_call_service`). **Over-transport latency measured 2026-08-14** (BENCHMARK_RESULTS.md §20): p50 188.79 µs / p95 295.23 µs / p99 373.51 µs vs 87.28 µs in-process — NPS-003 §6.1's <100 µs gate is NOT met at the median over the wire. **Rust transport hot path LANDED 2026-08-14** (ADR-0020 migration #6, `rust/transport/` + `ipc/transport_codec.py`): the per-message sendto/recvmsg syscall path with SCM_CREDENTIALS moves to Rust when the crate is built (Python floor fallback otherwise); the benchmark delta with it active is the close-path evidence. **Auto-maintained sender registry LANDED 2026-08-14** (`ipc/registry.py`): the backend keeps the pid → container mapping the server authenticates against in sync — each direct-syscall container's pid is registered at spawn and dropped on terminate, so services authenticate container senders with no manual bookkeeping (the container→service e2e proves the whole chain; the legacy `unshare(1)` path is untracked and fails closed, documented). **First real backend service LANDED 2026-08-14** (`ipc/service.py`, `BackendStatusService`): a container-facing status endpoint attached to the transport server — `ping` (whole-chain check) and `status` (capability-gated on `CAP_SYSTEM_INFO`, denied fail-closed without a `CapabilityManager`; reports backend version, uptime, and the caller's own identity and capability set). The server's CALL dispatch now swallows handler exceptions, so a service bug becomes an `internal error` reply instead of killing the serving thread; verified end-to-end through a real container (`test_container_calls_status_service`). **Runnable status-service daemon + auto capability lifecycle LANDED 2026-08-14:** the backend CLI (`nyrqis_backend.py service serve`) runs a `StatusServiceHost` — container manager, transport sender registry, capability manager, server, and service share state, so containers spawned against the daemon are automatically registered AND granted (defaults at spawn, revoked on terminate, NPS-010 §5) and can call the status service with zero manual bookkeeping; SIGINT/SIGTERM shut it down cleanly. **Control plane LANDED 2026-08-14:** the daemon's own user drives it over the same transport — a trusted-uid operator path on the server (container-FIRST, so daemon-spawned containers are never misattributed), `ServiceRouter` multi-service dispatch (payload `service` field, default `status`), and an operator-only `ControlService` (`container_run`/`container_list`/`container_kill`) reached via `nyrqis_backend.py control`; verified end-to-end with a real container spawned and killed through the wire. A shared-memory transport remains an alternative/complement (deferred).
