...
| CR-0037 | Repair the nyrqisctl payload surface: remove the stray unconditional `raise ValueError` that sat mid-`build_payload` since the CLI's first commit (4bb68bb) and made ~300 registered commands (audit chains, SLOs, GPU, RBAC, chaos, mesh, canary, capacity, tracing, topology, ...) crash with "unknown command" despite parsing fine; register the audit hash-chain summary under its own `audit-chain-summary` command (the chain variant had mutated alert-summary's parser via a reused variable, breaking `nyrqisctl alert-summary` outright); pin the whole parse→payload surface with a sweep test (`test_nyrqisctl_payload_surface.py`) so the class cannot recur silently | Accepted | — |
