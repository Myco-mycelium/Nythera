...
- [~] Debug tooling — `nyrqis debug` with step-through, breakpoints — **Phase A (incident bundle) + Phase C rider landed 2026-09-23** (`nyrqisctl debug bundle`: client-side composition of the existing health/status/containers/audit-log/nui-current ops — no new daemon surface; pinned by `test_debug_bundle.py`, 6/6 green; design note `DBG-001` v0.2.0 with deviations recorded); Phase B (attach with `CAP-DEBUG-ATTACH`, step-through/breakpoints) remains **Group-gated** per DBG-001 §4 — item stays open for B
