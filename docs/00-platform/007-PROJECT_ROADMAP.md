...
- [~] Debug tooling — `nyrqis debug` with step-through, breakpoints — **Phase A (incident bundle) + Phase C rider landed 2026-09-23, follow-ups closed the same day; Phase B DECIDED (D7: developer-mode manifests)** (`nyrqisctl debug bundle`: client-side composition of the existing health/status/containers/audit-log/nui-current ops — no new daemon surface; redaction default-on; per-container audit trails; `--chain-id` capture; pinned by `test_debug_bundle.py`, 9/9 green; DBG-001 v0.4.0). **D7 chose the in-container design**: a `debug: true` manifest class + `CAP-DEBUG-ATTACH`, debugpy/gdbserver inside the debugged container, ptrace relaxation for that class only, NPS-021 addendum BEFORE implementation. Implementation progress (2026-09-23): the NPS-021 v1.1.0 addendum and NPS-011 v1.4.0 landed as D7 preconditions; a §4.1 design review mapped all five §5.5 MUSTs to enforcement sites; **the launcher manifest-class plumbing is LANDED** (`debug_class` manifest field, evaluation-time rejection per §4.4, centralized class-conditional grant guard, construction-time seccomp gate, class-through-policy-file launcher parity, state-surface visibility — pinned by `test_debug_manifest_class.py`, 17 tests). **2026-09-25: the remaining D7 work items LANDED** — debug-image staging (`_setup_debug_staging`: per-container dir on the writable overlay at spawn, loopback-pinned `debug-endpoints.json` per §5.5 req 4, non-fatal failure leaves no staging) and the `containers debug` op family (`container_debug` info/attach/detach/bind: CAP_DEBUG_ATTACH required — operator-only per §5.5 req 1 — RUNNING state, staging fail-closed, bind additionally gated on CAP_NETWORK_BIND, accepted actions audit-chained with the manifest class in the entry result per FIND-CAPABILITY-006 req 5; IPC `container_debug` op + CLI `containers debug-info/attach/detach/bind`, pinned by the payload-surface sweep). Pinned by `test_debug_manifest_class.py`, 28 tests. The item stays `[~]` only for the interactive attach UX (a live debugpy/gdbserver session needs a running container to validate); the security surface — gates, audit, endpoints — is complete. **Repair bonus (CR-0037):** the CLI shadow investigation exposed a stray mid-function raise in `build_payload` that had made ~300 registered commands crash with "unknown command" since the CLI's first commit — removed, the audit-chain summary registered properly (`audit-chain-summary`), `alert-summary` un-hijacked, the whole parse→payload surface pinned by a sweep test (`test_nyrqisctl_payload_surface.py`)
