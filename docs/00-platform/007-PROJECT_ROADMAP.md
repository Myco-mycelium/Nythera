...
- [ ] IDE integration — VS Code extension for .nstudio editing (no extension source, `.vsix`, package.json, or LSP code found in the 2026-09-23 audit — the item is genuinely absent; building blocks that DO exist: the `nui-validate` control op and the `nyq` SDK CLI)
