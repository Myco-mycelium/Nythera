...
- [~] Debug tooling — `nyrqis debug` with step-through, breakpoints — **Phase A (incident bundle) + Phase C rider landed 2026-09-23, follow-ups closed the same day** (`nyrqisctl debug bundle`: client-side composition of the existing health/status/containers/audit-log/nui-current ops — no new daemon surface; **redaction default-on** (`--redact/--no-redact`) strips vault aggregates from status/health; per-container audit trails (the op requires a container_id — a wire contract the scripted tests had masked); `--chain-id` drives the existing summary+verification ops — no chain-listing op exists, so ids are operator-supplied; pinned by `test_debug_bundle.py`, 9/9 green; DBG-001 v0.3.1); Phase B (attach with `CAP-DEBUG-ATTACH`, step-through/breakpoints) remains **Group-gated** per DBG-001 §4 — item stays open for B. **Repair bonus (CR-0037):** following the recorded CLI shadow to its root exposed a stray mid-function raise in `build_payload` that had made ~300 registered commands crash with "unknown command" since the CLI's first commit — removed, the audit-chain summary registered properly (`audit-chain-summary`), `alert-summary` un-hijacked, and the whole parse→payload surface pinned by a sweep test (`test_nyrqisctl_payload_surface.py`)
