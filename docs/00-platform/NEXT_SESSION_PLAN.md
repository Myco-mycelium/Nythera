...
| Next plan cycle | Design-language phase 2 + registry/Nyforge contract work closed. Lead candidates: (a) real-hardware probe results → MISSING work items (standing — needs a hardware run), (b) Nyforge Inspector UI panel on `inspect_version` (version badge, loss list with change spans, unknown-requirement warnings), (c) expose the pill variant as a runtime-selectable shell theme, (d) optional: Rust/FFI parity for the version preflight |
