...
| **IDE integration: the prototype landed — the gap closed honestly, the packaging half stays open** | ✅ `ide/vscode-nyrqis/` — a real extension, not a placeholder: manifest declares the `nstudio` language (`.nstudio`), `nyrqis.validateDocument` + `nyq new/build/test` palette commands, and `validateOnSave` config. Validation is REAL: `nst_validate.py` runs the pure-Python import-gate floor (`ui/nstudio.loads` — the same NFS-001 contract tables the daemon's operator-only `nui_validate` enforces on import), so editor squiggles match import behavior exactly; the extension deliberately does NOT round-trip the daemon (the gate is operator-only — an editor must not need operator authority for squiggles). Pinned by `test_ide_extension.py` (4 tests): manifest contract, a node mock-harness that loads extension.js and drives the diagnostic conversion, and the validator against a real shipped fixture (clean → `[]`/0, bad version → `schema-version` diagnostic/1). Roadmap item → `[~]`: the `.vsix` packaging/marketplace half remains open (needs tooling this repo doesn't have) |
