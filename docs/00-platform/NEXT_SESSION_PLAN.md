...
| **The .vsix build half of the IDE-integration item** | ✅ `tools/build_vsix.py` (pinned by 12 contract tests in `test_build_vsix.py`, sandbox trees only): the vscode-nyrqis extension packages into a real, installable .vsix with the Python standard library alone — no vsce, no npm registry, no network; fail-closed (missing manifest fields/entry point refuse; the written archive is reopened and verified) and deterministic (fixed timestamps → byte-identical rebuilds). Verified beyond the tests: real VS Code accepted the artifact (`--install-extension` success; `--list-extensions` shows `nyrqis.vscode-nyrqis` in a throwaway extensions dir). Artifacts gitignored; CHANGELOG 0.29.34 + pyproject bumped (drift OK). Marketplace publish stays open — an owner-side publisher account, not tooling. Roadmap stays `[~]` with the build half struck |
