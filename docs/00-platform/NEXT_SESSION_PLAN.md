...
| **Dispatch probe with scope evidence; artifacts kept** | ✅ third dispatch attempt 403, and the response carries NO X-OAuth-Scopes header — the credential is a FINE-GRAINED PAT (missing Actions:write), confirming the carried rotation trigger with evidence instead of inference; disk cleanup decided BY USER: keep all ~2.8 GB of ~/nyrqis-work (rootfs trees, .deb caches, ISOs) |
