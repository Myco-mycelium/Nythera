...
| Next plan cycle | Design-language phase 2 + registry/Nyforge contract work closed — every candidate shipped, including persistence. (a) real-hardware probe results → MISSING work items: THE standing item, needs a hardware run. Open next-thread candidates: a Nyforge-Inspector *side panel* inside the editor itself, Rust/FFI parity for `inspect_version`, and per-screen variant mixing (pill dock on a stock desktop) |
