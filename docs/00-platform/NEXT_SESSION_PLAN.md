...
| Next plan cycle | Design-language phase 2 + registry/Nyforge contract work closed — every candidate shipped. (a) real-hardware probe results → MISSING work items: THE standing item, needs a hardware run; (b)–(e) all ✅ done (Inspector panel, runtime variants + boot-menu entry, registry parity FFI, hot-swap + settings-panel picker). Open next-thread candidates: persistence (remember the chosen variant across boots), a Nyforge-Inspector *side panel* inside the editor itself, and Rust/FFI parity for `inspect_version` |
