...
| **The .vsix build half of the IDE-integration item** | ✅ `tools/build_vsix.py` (pinned by 13 contract tests in `test_build_vsix.py`, sandbox trees only; 12 originally, the 13th added when the determinism claim was caught false and fixed): the vscode-nyrqis extension packages into a real, installable .vsix with the Python standard library alone — no vsce, no npm registry, no network; fail-closed (missing manifest fields/entry point refuse; the written archive is reopened and verified) and deterministic (fixed member timestamps + SOURCE_DATE_EPOCH CreationDate → byte-identical rebuilds across second boundaries). Verified beyond the tests: real VS Code accepted the artifact (`--install-extension` success; `--list-extensions` shows `nyrqis.vscode-nyrqis` in a throwaway extensions dir). Artifacts gitignored; CHANGELOG 0.29.34 + pyproject bumped (drift OK). Marketplace publish stays open — an owner-side publisher account, not tooling. Roadmap stays `[~]` with the build half struck |
| **Release v0.29.34 shipped + verified end-user-style** | ✅ tag pushed (annotated, 9a9ca20); release created via API (ID 397251990) with the boot-smoke line WITHHELD until proven (the v0.29.33 lesson: publish claims only after verification), PATCHed in after all four paths passed; tag workflows success (live-iso #36246338154 ~8 min, live-iso-arm64 #36246338155 ~38 min); both assets downloaded UNAUTHENTICATED byte-exact (258342912 / 267290624 bytes; sha256 5ba9470f…681c392c amd64 / f867ceb8…1c01a22e arm64); ALL FOUR boot smokes PASS serially on the DOWNLOADED ISOs (exit 0 each) |
