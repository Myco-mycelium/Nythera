...
| Next plan cycle | Design-language phase 2 + registry/Nyforge contract work closed. Lead candidates: (a) real-hardware probe results → MISSING work items (standing — needs a hardware run), (b) ~~Nyforge Inspector UI panel~~ ✅ done (`ui/inspector_panel.py`), (c) ~~expose the pill variant as a runtime-selectable shell theme~~ ✅ done (`--variant` / `NYRQIS_SHELL_VARIANT`), (d) ~~optional: Rust/FFI parity for the version preflight~~ ✅ done differently (`swap_document` + registry parity FFI check), (e) ~~hot-swap a shell variant into a live session without restart~~ ✅ done (`NyforgeBridge.swap_document`); open: Nyforge UI wiring `swap_document` behind a variant-picker button |
