...
| **The pre-read is drafted** | ✅ `AG_BRIEF_BUILD_ARCH_DUALITY.md` (suggest-side, D3/D4 brief format): verified content analysis — the two bodies have DIFFERENT centers of gravity (BUILD-001 = policy: reproducibility MUSTs, CI budgets, signing; BUILD-ARCH = practical reference: crate graph, per-crate tables, build commands), with a contradiction table (Python floor 3.12+ vs 3.10, no-venv vs venv, Windows target present vs absent, stale 2026-09-01 test counts) and the two unsanctioned-flip precedents laid out (ADR-0019 revert vs ADR-0022/0023 ratify). Three options with consequences ledgers; **recommends Option A** — BUILD-001 canonical (zero citation breakage), absorb BUILD-ARCH's practical content (the merge itself refreshes the stale tables), remove the copy, dissolve the Accepted marking with the file. Downstream touch-points enumerated so the session can size the work; nothing blocks the decision |
