...
- **`test_backend.py`** — `TestContainerCapabilityLifecycle` (6 tests: spawn init, spawn-failure revoke, terminate revoke, wait legacy + direct revoke, no-manager no-op) and `TestStatusServiceHost` (4 tests: host serves status over a real socket, host manager auto-grants, CLI wiring via a mocked host, and a REAL `service serve` subprocess that binds 0700 and exits 0 on SIGTERM). The status e2e now proves the whole chain automatically — the container is registered AND granted by its manager, no manual `initialize_container`. Suite **265 → 275** (249 run + 26 skipped).
