...
- **Four more desktop-specific primitives** join the registry's Shell
  category (24 Shell types total — doc #15's list is complete), each
  with a typed semantic contract: `AppGrid` (apps/columns/iconSize,
  `appClicked` → `Launch`), `Clock` (`format` 12h/24h, `showSeconds`),
  `Dock` (`position`/`pinnedApps`/`runningApps`/`autoHide`/`iconSize`/
  `magnify`, `appClicked` → `Launch`), and `TitleBar` (`title`/`icon`,
  `doubleClicked`).
- **All three consumers pick the vocabulary up automatically** — the
  Rust crate embeds the same registry file at compile time, and
  Nyforge's C# tables are regenerated by `tools/generate_contracts.py`
  from the vendored copy (contracts-synced CI gate green).
- **The desktop fixture exercises every new type:** the taskbar clock
  is a real `Clock` bound to a new `clockFormat` state, a `Dock` with
  pinned/running apps sits on the desktop surface, a `Launcher` overlay
  hosts an `AppGrid` of all apps (Search submit opens the Command
  Palette), and a real Files window is framed by `WindowFrame` +
  `TitleBar` + `WindowControls` whose close button runs the window's
  `Close` action. `desktop.nstudio` is now 37 components / 10 behaviors
  / 6 bindings.
- **Fail-closed on both gates:** unknown properties/events on the new
  types produce byte-identical messages in the floor and the crate
  (differential), and Nyforge's validator (ER-NUI-020 contract checks)
  and Inspector definitions regenerate alongside.
- **Tests:** `TestShellComponents` (5) + 2 crate conformance cases;
  the fixture-shape test asserts the new counts. Suite 635 → **642**.
